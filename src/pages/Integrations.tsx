...
export default function Integrations() {
  ...
  const connectGitHub = () => {
    toast.info('تكامل GitHub يحتاج OAuth App وعمليات callback server-side. لم يتم تفعيل اتصال وهمي.')
  }

  const connectTelegram = () => {
    const token = prompt('أدخل Bot Token الخاص بـ Telegram:')
    if (token) {
      toast.info('لم يتم حفظ Bot Token لأن مسار Telegram الآمن غير مهيأ في هذا الإصدار.')
    }
  }
  ...
  const testMcpConnection = async (index: number) => {
    setTestingMcp(index)
    setTestingMcp(null)
    toast.info(`لم يتم اختبار ${mcpServers[index].name}: اتصال MCP يجب أن يمر عبر الخادم وليس المتصفح.`)
  }
  ...
  const executeMcpTool = (serverIndex: number, tool: any) => {
    ...
    toast.info(`الأداة ${tool.name} محفوظة كتعريف فقط. يلزم endpoint خادمي لتنفيذها بأمان.`)
  }
