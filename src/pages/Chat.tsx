...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    if (!chat) return
    setCurrentChat(chat); setMode(chat.mode)
  ...
  const switchMode = (newMode: 'chat' | 'agent') => { setMode(newMode); if (currentChat && user) void updateChat(currentChat.id, user.id, { mode: newMode }).then(chat => { setCurrentChat(chat); setChats(prev => prev.map(item => item.id === chat.id ? chat : item)) }).catch(error => toast.error(error.message)) }
  ...
  return <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
    ...
    <div className="flex-1 flex flex-col min-w-0"><div className="h-14 border-b border-dark-700 px-5 flex items-center justify-between bg-dark-900 flex-shrink-0 gap-3"><div><div className="font-semibold text-lg truncate">{currentChat?.title || 'محادثة جديدة'}</div><div className="text-xs text-dark-500">{selectedProvider?.name || 'اختر مزودًا'} • {selectedModel || 'اختر نموذجًا'} • {mode === 'agent' ? 'وضع الوكيل' : 'دردشة'}</div></div><div className="flex items-center gap-2"><div className="flex items-center bg-dark-800 rounded-2xl p-1 text-xs"><button onClick={() => switchMode('chat')} className={`px-3 py-1.5 rounded-xl ${mode === 'chat' ? 'bg-white text-dark-950' : 'text-dark-400'}`}>دردشة</button><button onClick={() => switchMode('agent')} className={`px-3 py-1.5 rounded-xl flex items-center gap-1 ${mode === 'agent' ? 'bg-white text-dark-950' : 'text-dark-400'}`}><Bot size={14} /> وكيل</button></div><div className="relative group"><button className="flex items-center gap-2 text-sm px-3 py-2 bg-dark-800 rounded-2xl border border-dark-700">{selectedProvider?.name || 'اختر مزود'}<ChevronDown size={14} /></button><div className="absolute left-0 mt-2 w-64 bg-dark-900 border border-dark-700 rounded-2xl shadow-2xl py-1 z-50 hidden group-hover:block">{providers.map(provider => <div key={provider.id} onClick={() => { setSelectedProvider(provider); setSelectedModel(provider.model || provider.models?.[0] || '') }} className="px-4 py-2.5 hover:bg-dark-800 cursor-pointer text-sm flex justify-between">{provider.name}<span className="text-[10px] text-emerald-400">{provider.status}</span></div>)}</div></div></div></div>
